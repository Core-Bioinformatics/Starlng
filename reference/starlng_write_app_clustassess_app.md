...
togheter with the content of the ClustAssess Shiny app and generates a
...
- ca_app_folder:
  ...
  The path of the folder containing the files associated with the
  ClustAssess Shiny app.
...
- stable_feature_set_size:
  ...
  The size of the feature set that is chosen from the ClustAssess
  analysis.
...
- stable_n_clusters:
  ...
  The number of clusters that is chosen from the ClustAssess analysis.
  If NULL, the function will use all the clusters that are obtained.
  Defaults to NULL.

- use_all_genes:

  If TRUE, uses all the genes in the expression matrix. If FALSE, uses
  only the stable feature set as determined by ClustAssess. Defaults to
  TRUE.
